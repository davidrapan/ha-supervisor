...
async def test_api_network_enable_ipv6(api_client: TestClient, coresys: CoreSys):
    """Test setting supervisor country via API."""
    assert coresys.config.bridge_ipv6 is None

    resp = await api_client.post("/docker/options", json={"enable_ipv6": "True"})
    assert resp.status == 200

    assert coresys.config.bridge_ipv6 is True
